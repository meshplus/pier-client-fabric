...
func (c *Client) Initialize(configPath, selfMethod string, extra []byte) error {
	...
	mgh, err := newFabricHandler(contractmeta.EventFilter, eventC, selfMethod)
	if err != nil {
		return err
	}
...
func (c *Client) polling() {
	for {
		...
		case <-c.ticker.C:
			...
			for _, ev := range evs {
				ev.Proof = proof
				c.eventC <- ev.Convert2IBTP(c.selfMethod, pb.IBTP_INTERCHAIN)
				if c.outMeta == nil {
					c.outMeta = make(map[string]uint64)
				}
				c.outMeta[bitxid.DID(ev.DstContractDID).GetMethod()]++
			}
		case <-c.done:
...
func (c *Client) unpackIBTP(response *channel.Response, ibtpType pb.IBTP_Type, proof []byte) (*pb.IBTP, error) {
	...
	ret.Proof = proof
	return ret.Convert2IBTP(c.selfMethod, ibtpType), nil
}
